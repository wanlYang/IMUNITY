layui.use(['form', 'layer', 'laydate','upload'], function() {
	...
	form.on("submit(addProduct)", function(data) {
		...
		$.ajax({
			type: "POST",
			url: getRealPath() + "/admin/manager/shop/product/submit/add",
			data: data.field,
			success: function(result) {
				if(result.status == 200) {
					setTimeout(function() {
	...
	upload.render({
		...
		before: function(obj) {
			...
			obj.preview(function(index, file, result) {
				$('#show').append('<div style="height: 210px;width: 210px;float: left;margin-left: 10px;"><img src="' + result
					+ '" alt="' + file.name
